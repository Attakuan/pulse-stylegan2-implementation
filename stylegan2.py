    ...
    def forward(self, x, img, ws, highres_outs=None, return_f = False, return_styles=False,
        ...
        if self.in_channels == 0:
            ...
            if x.shape[-1] == self.embed_res and highres_outs is not None: 
                ...
                # Concat Skip Connections with Generator F features. (E1)
                aligned_feats = self.modify_feature_alignment(highres_outs[f'{self.embed_res}x{self.embed_res}'], highres_outs['F_feats'])
                aligned_feats = self.modify_feature_extraction(aligned_feats)
                # Adopt to random edits (E2)
